...
export default function Skeleton({ loading, avatar, isChange, title, row, width, data }: Props) {
  let dataSource = data
    ? data
    : [
        {
  ...
  return (
    <div>
      <div
        ...
        style={{ display: Loading.isChange ? 'inline-block' : 'none' }}
      >
      ...
      <div style={{ width: '800px' }}>
        <div
          className={Loading.loading ? 'fl-concis-list-item' : 'fl-concis-list'}
          ...
          {dataSource.map((item: any) => {
            return (
              <div className="fl-concis-skeleton" key={item.id}>
                {/* 头像框 */}
                <div
                  className={Loading.loading ? 'fl-concis-avatar' : 'fl-concis-skeleton-avatar'}
                  style={{ display: avatarIsShow }}
                >
                  {Loading.loading ? 'A' : ''}
                </div>
                {/* 数据框 */}
                <div
                  className={Loading.loading ? 'fl-concis-text' : 'fl-concis-skeleton-container'}
                >
                  <div
                    className={
                      Loading.loading
                    ...
                    style={{ display: titleIsShow }}
                  >
                    {Loading.loading ? item.title : ''}
                  </div>
                  {Loading.width.map((item, index) => {
                    return (
                      <div
                        className="fl-concis-skeleton-container-line"
                        style={{ width: item }}
                        key={index}
                      ></div>
                    );
                  ...
                  <div
                    className={
                      Loading.loading ? 'fl-concis-text-cont' : 'fl-concis-skeleton-container-cont'
                    }
                  >
